...
dist-rpm: dist
	...
	cp synergy-1.3.1.tar.gz $(RPMTOPDIR)/SOURCES
...
dist-pkg: all
	rm -rf $(PKGTOPDIR)
	mkdir $(PKGTOPDIR)
	mkdir $(PKGTOPDIR)/synergy-1.3.1
	mkdir $(PKGTOPDIR)/synergy-1.3.1/doc
	cp $(PKG_FILES) $(PKGTOPDIR)/synergy-1.3.1
	cp $(PKG_DOC_FILES) $(PKGTOPDIR)/synergy-1.3.1/doc
	(cd $(PKGTOPDIR)/synergy-1.3.1; \
		chmod 644 *; \
		chmod 755 doc $(PKG_PROG_FILES); \
		strip $(PKG_PROG_FILES) )
	type=`uname -s -m | tr '[A-Z] ' '[a-z].'`; \
	(cd $(PKGTOPDIR); tar cf - synergy-1.3.1 | \
		gzip - ) > synergy-1.3.1-1.$${type}.tar.gz && \
	rm -rf $(PKGTOPDIR)
